...
async function importFile(file: File) {
  try {
    ...
    profile = imported; error = "资料已保存到本地资料库"; render();
  } catch (cause) { error = cause instanceof Error ? cause.message : "资料导入失败"; render(); }
}

function exportFile() {
  if (!profile) { error = "尚未加载档案，请先导入资料"; render(); return; }
...
function render() {
  root.replaceChildren(
    h("header", {}, [h("div", { class: "brand" }, [h("span", { class: "mark", text: "✓" }), h("strong", { text: "ApplyPilot 资料中心" })])]),
    h("section", { class: "card" }, [
      h("h1", { text: "本地资料库" }),
      h("p", { class: "muted", text: "资料保存在此浏览器本地。导出前请确认文件保存位置。" }),
      h("div", { class: "actions" }, (() => {
        const input = h("input", { type: "file", accept: ".yaml,.yml,.json" }) as HTMLInputElement;
        input.onchange = () => { const file = input.files?.[0]; if (file) void importFile(file); };
        const exportButton = h("button", { class: "secondary", text: "导出 YAML" }) as HTMLButtonElement;
        exportButton.onclick = exportFile;
        return [input, exportButton];
      })())
    ]),
    h("section", { class: "card" }, [
      h("div", { class: "card-title", text: profile ? `已加载：${profile.identity?.name || profile.profile_id}` : "尚未加载档案" }),
      h("p", { class: "muted", text: profile ? `教育 ${profile.education?.length || 0} 条 · 经历 ${profile.experiences?.length || 0} 条 · 附件 ${profile.assets?.length || 0} 个` : "导入现有 profile.yaml 或 profile.json 后，可以在侧栏中填写当前页面。" })
    ]),
    ...(error ? [h("div", { class: "error", text: error })] : [])
  );
}
