...
在上一轮修复基础上，继续修复并加入回归测试：完整字段描述变化检测与动态重扫、无效选填项阻断、每次运行更新披露上下文、终态应用防重跑、异常可恢复检查点、第三方联系人和模糊附件保守映射、资产类型校验、终端回写验证、编码性别选项、JSON 导入输出、内容派生 profile ID、真实表单快照和本地敏感文件权限；本轮又补上披露策略预填绕过、运行创建异常、checkpoint URL、复杂组件候选项回读、禁用提交按钮、失败事件脱敏、模型 ID 冲突、动态表单渲染等待和登录/未知页面人工接管重扫。Python 3.11 与 3.14 强制浏览器全量测试均为 **407 passed**。

## 交互边界补充修复（2026-09-13）

新增回归覆盖并修复了本轮深审确认的 11 个问题：同源 iframe 控件扫描与检测脚本执行、登录后重选最新弹窗页面和平台适配器、只按真实阶段跳转计算上限、表单检查失败安全暂停、非法 `FillResult` 安全暂停、避免用 `TypeError` 重试填充器、登录帮助文案误判、Moka/北森搜索选项误点全局 `li`、忽略 `userId` 等易变参数的岗位 ID、别名归一化后的画像 ID，以及审计快照和事件中的 URL 参数脱敏。画像 ID 现在以稳定身份字段派生，补充资料不会改变现有申请关联。

验证：Python 3.14 与 3.11 强制浏览器全量测试均为 **407 passed**。真实招聘站点仍需人工验证，流程不会自动点击最终提交。

## URL 配置与慢速导航修复（2026-09-14）

针对从聊天窗口复制 Markdown 链接导致 `Page.goto` 收到 `[标题](URL)` 的问题，`apply run` 现在支持应用目录的 `config.yaml` 中配置 `job_url`，并对命令行输入执行严格的 http(s) 校验、Markdown 解包和 `\\&` 还原。51job 等慢速页面的导航等待从固定 10 秒改为 30 秒，并在 `domcontentloaded` 后继续处理，避免无关资源阻塞整个网申流程。新增配置、URL 清洗和导航参数回归测试。

验证：Python 3.14 与 3.11 强制浏览器全量测试均为 **413 passed**。
...
验证：Python 3.11 与 3.14 下强制浏览器测试的全量套件通过（407 项）。本报告保留原始复现和根因，作为后续回归与设计依据。
