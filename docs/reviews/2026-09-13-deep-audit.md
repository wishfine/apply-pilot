...
在上一轮修复基础上，继续修复并加入回归测试：完整字段描述变化检测与动态重扫、无效选填项阻断、每次运行更新披露上下文、终态应用防重跑、异常可恢复检查点、第三方联系人和模糊附件保守映射、资产类型校验、终端回写验证、编码性别选项、JSON 导入输出、内容派生 profile ID、真实表单快照和本地敏感文件权限；本轮又补上披露策略预填绕过、运行创建异常、checkpoint URL、复杂组件候选项回读、禁用提交按钮、失败事件脱敏、模型 ID 冲突、动态表单渲染等待和登录/未知页面人工接管重扫。Python 3.11 与 3.14 强制浏览器全量测试均为 **392 passed**。
...
验证：Python 3.11 与 3.14 下强制浏览器测试的全量套件通过（392 项）。本报告保留原始复现和根因，作为后续回归与设计依据。
