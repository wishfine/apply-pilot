...
验证：Python 3.14 下强制浏览器测试的全量套件通过（359 项）。本报告保留原始复现和根因，作为后续回归与设计依据。

现有完整测试在 Python 3.14 下为 **344 passed（22.38 秒）**，强制启用了浏览器回归。额外检查使用真实 Chrome 的本地合成页面、临时 SQLite、虚构档案及 CliRunner；未访问真实招聘表单、未提交申请、未调用外部模型。本次仅审查，不修改业务代码。
...
本记录的 9 项均有运行证据，不以“未实现完整平台支持”替代具体故障。现有 344 项测试没有覆盖上述输入；应把这些复现变成回归断言，而不是只增加成功路径测试。
